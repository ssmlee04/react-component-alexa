...
export class AlexaRanks extends React.Component {
  ...
  render() {
    ...
    const data = {
      labels: profile.alexa.data.map(d => dayjs.utc(d.ts).format('YYYYMM')),
      datasets: [{
        yAxisID: '1',
        type: 'line',
        fill: false,
        ...
        pointRadius: 2,
        pointHoverRadius: 2,
        data: profile.alexa.data.map(d => d.rank),
    ...
    return (
      <div style={{ width: '100%', padding: 5, fontSize: 14 }}>
        <div style={{ color: 'darkred', fontWeight: 'bold' }}>{profile.ticker} - {profile.name} <span className='green'>Website Ranking</span></div>
        <Bar data={data} height={170} options={options} />
        <div style={{ fontSize: 12, color: 'gray' }}>Generated by <span style={{ color: 'darkred' }}>@earningsfly</span> with <span style={{ fontSize: 16, color: 'red' }}>❤️, datasource: alexa.com</span></div>
      </div>
    );
  }
}
